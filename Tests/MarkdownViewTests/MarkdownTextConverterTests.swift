...
struct MarkdownTextConverterTests {
    ...
    @Test("Embeds a custom task list marker")
    @MainActor
    func embedsCustomTaskListMarker() {
        let textContent = Self.makeTextContent(
            markdown: "- [x] Done",
            taskListMarker: AnyTaskListMarker { _ in
                Image(systemName: "checkmark.square")
            }
        )

        #expect(textContent.embeddedViewCount == 1)
        #expect(textContent.plainText == "\u{FFFC} Done")
    }
...
private extension MarkdownTextConverterTests {
    ...
    @MainActor static func makeTextContent(
        ...
        taskListMarker: AnyTaskListMarker? = nil,
